...
.controller('dashboardCtrl',function($scope,$sce,$location,$filter,$window,Call,Agent){
	...
	// Function that returns all the agents for the campaign we are currently
	// looking.
	...
	// When the agent is changed, we update the url query string
...
.filter('filter', function(){
	return function(input, params){
		...
		for (var i = 0; i < input.length; i++){
			// If agent is not defined, or the call has correct agent, this will pass through
			if(params.agent && !(params.agent == input[i].agent_name)){continue;}
